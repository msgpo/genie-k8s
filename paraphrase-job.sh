...
parse_args "$0" "owner dataset_owner project experiment \
						input_dataset input_split output_dataset filtering_model paraphrasing_model_full_path skip_generation only_paraphrase task_name" "$@"
shift $n
...
elif [ "$task_name" = "almond_multilingual" ] ; then
	export is_multilingual=true
...
run_paraphrase(){
	...
	elif [ "$is_multilingual" = true ] ; then
		genienlp run-paraphrase \
			--model_name_or_path paraphraser \
			--input_file ${input_dir}/${input_split}.tsv \
			--output_file ${output_dir}/paraphrased.tsv \
			--input_column 0 \
			$paraphrasing_arguments
	else
