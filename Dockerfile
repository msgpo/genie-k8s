...
#ARG TRANSFORMERS_VERSION=master
#RUN git clone https://github.com/Mehrad0711/transformers.git /opt/transformers/
#WORKDIR /opt/transformers/
#RUN git fetch && git checkout ${TRANSFORMERS_VERSION} && pip3 install -e . && pip3 install -U pytorch_lightning && pip3 install wandb rouge_score
...
WORKDIR /opt/genie-toolkit/
COPY lib.sh generate-dataset-job.sh train-job.sh evaluate-job.sh paraphrase-job.sh train-paraphrase-job.sh translate-job.sh transformers-job.sh ./
...
#USER root
#WORKDIR /opt/transformers/examples/seq2seq
#WORKDIR /home/genie-toolkit
